...
class MODRENAMER_PT_MainPanel(bpy.types.Panel):
    ...
    def draw_text_element_properties(self, layout, element, element_index):
        ...
        # Item controls
        col = row.column(align=True)
        col.operator("modrenamer.move_text_item_up", text="", icon='TRIA_UP').element_index = element_index
        col.operator("modrenamer.move_text_item_down", text="", icon='TRIA_DOWN').element_index = element_index
        col.separator()
        col.operator("modrenamer.remove_text_item", text="", icon='REMOVE').element_index = element_index
...
class MODRENAMER_OT_MoveElementUp(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if pattern.active_element_index <= 0 or pattern.active_element_index >= len(pattern.elements):
            return {'CANCELLED'}
...
class MODRENAMER_OT_MoveElementDown(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if pattern.active_element_index < 0 or pattern.active_element_index >= len(pattern.elements) - 1:
            return {'CANCELLED'}
...
class MODRENAMER_OT_MoveTextItemUp(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if element.active_item_index <= 0 or element.active_item_index >= len(element.items):
            return {'CANCELLED'}
...
class MODRENAMER_OT_MoveTextItemDown(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if element.active_item_index < 0 or element.active_item_index >= len(element.items) - 1:
            return {'CANCELLED'}
