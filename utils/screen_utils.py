...
def resolve_window(
    identifier: Union[int, str, Window, None], context: Context
) -> Optional[Window]:
...
def resolve_screen(identifier: Union[str, Screen, None]) -> Optional[Screen]:
...
def resolve_area(identifier: Union[str, Area, None], screen: Screen) -> Optional[Area]:
...
def resolve_region(
    identifier: Union[int, str, Region, None], area: Area
) -> Optional[Region]:
...
def find_areas_by_type(screen: Screen, area_type: str) -> List[Area]:
    """指定されたタイプのエリアを全て見つけます。

    Args:
        screen: 対象のスクリーン
        area_type: エリアタイプ（例：'VIEW_3D', 'OUTLINER'）

    Returns:
        List[Area]: 見つかったエリアのリスト
    """
    return [ar for ar in screen.areas if ar.type == area_type]
