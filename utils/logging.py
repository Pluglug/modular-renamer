...
class LoggerPreferences(PropertyGroup):
    ...
    def update_logger_settings(self, context=None):
        ...
        if self.log_to_file and not self.log_file_path:
            addon_name = context.preferences.active_addon.module
...
def register():
    ...
    for module_name, logger in mods.items():
        pr.logger_prefs.register_module(module_name, "DEBUG")

    pr.logger_prefs.update_logger_settings(bpy.context)
